...
def test_model():
    ...
    # Assuming test_data is your input test data
    test_data = X_test.iloc[0]  # Taking the first row of the test data
    
    # Reshape the data to match the input shape required by the model
    test_data = test_data.values.reshape(1, -1)
    
    # Load the saved model
    loaded_model = joblib.load('random_forest_model_checkpoint.pkl')
    
    # Predict the university for the input test data
    predicted_university = loaded_model.predict(test_data)
    
    print("Predicted University:", predicted_university)
